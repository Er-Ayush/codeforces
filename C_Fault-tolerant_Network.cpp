...
for(int i=0;i<n;i++){
    w.push_back(abs(b[i]-p));
    x.push_back(abs(b[i]-q));
}
for(int i=0;i<n;i++){
    y.push_back(abs(a[i]-r));
    x.push_back(abs(a[i]-s));
}
int mip=*min_element(poora(a)),miq=*min_element(poora(a)),mir=*min_element(poora(b)),mis=*min_element(poora(b));
int ip=0,iq=0,ir=0,is=0;
for(int i=0;i<n;i++){
if(a[i]==mip){
ip=(i+1);
break;
