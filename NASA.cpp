...
int ans=0;
    ...
    for (int i = 0; i < A.size(); i++) {
        ...
        // Calculate XOR with previously visited numbers
        for (auto& i : freqMap) {
            int xorValue = currNum ^ i.first;

            // Check if XOR value has an even number of differing bits
            if (__builtin_popcount(xorValue) % 2 == 0) {
                count += i.second;
            }
        }
